(() => {
  ...
  function initTabs() {
    ...
    tabs.forEach(tab => {
      tab.addEventListener('click', () => {
        ...
        accountPanel.hidden = target !== 'account';
        teamsPanel.hidden = target !== 'teams';
  ...
  function initAccount(session) {
    ...
    document.getElementById('signOutBtn').addEventListener('click', () => ds.signOut());
  }
      ...
      members.forEach(m => {
        ...
        if (team.role === 'owner' && m.user_id !== session.user.id) {
          ...
          removeBtn.addEventListener('click', (e) => {
            ...
            sb.from('team_members').delete()
              .eq('team_id', team.id).eq('user_id', m.user_id)
              .then(() => openTeamModal(team))
  ...
  function closeTeamModal() {
    document.getElementById('teamModalOverlay').hidden = true;
    currentTeam = null;
  }
  ...
  function initTeams() {
    ...
    document.getElementById('teamModalDone').addEventListener('click', closeTeamModal);
